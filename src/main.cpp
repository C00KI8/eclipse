...
#if defined(NDEBUG)
# error "Eclipse cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xda7b139cc88ee27c5012a9a55f21636fbe5cdadd05d9e0dd87e5102a88dde8b9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eclipse: starting difficulty is 1 / 2^12
...
// Constant stuff for coinbase transactions we create:nheight
CScript COINBASE_FLAGS;

const string strMessageMagic = "Eclipse Signed Message:\n";
...
{
    // Eclipse: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 140000); // Eclipse: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Eclipse: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Eclipse: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eclipse: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eclipse: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd5277f68c69592aabe8d2fac53641b79a96b1af2fbe82b71e9669689e93ba3fc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "08/June/2020";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e832d704b7733e5b2ed64fef1c7a539107cd36c012a41f24c77be751ffe9ba8b04c4415cb37f3061da8993c38e4545c6c6a1cecbbd4dcea494e1ec2c5e56afac") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1591571049;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085120171;

        if (fTestNet)
        {
            block.nTime    = 1591570967;
            block.nNonce   = 387985275;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4ed57bca77dea0657b0843b86c3059be21331fcd7f8e82a36baa19bfcd6e224e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Eclipse: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EclipseMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EclipseMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EclipseMiner(CWallet *pwallet)
{
    printf("EclipseMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eclipse-miner");
    ...
    try { loop {
        ...
        printf("Running EclipseMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EclipseMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EclipseMiner, pwallet));
}
